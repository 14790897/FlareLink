...
说明: 高压发生器上通常有一个4脚开关，将MOSFET的S-D连接到
      开关的对角线上（如1-3或2-4），MOSFET导通时相当于短路
      这两个对角接线柱，从而触发高压发生器工作。

⚠️ 重要：
   1. 高压发生器电源负极必须与ESP32-C3的GND连接（共地）
   2. MOSFET源极(S)接ESP32 GND
   3. MOSFET漏极(D)接高压发生器开关的一个对角接线柱
   4. 开关的另一个对角接线柱也接ESP32 GND
   
⚠️ 已知问题：
   使用MOSFET方案时可能出现ESP32死机/重启，疑似高压发生器的
   电流倒灌导致。建议添加光耦隔离或改用继电器方案（更可靠）。
```
...
继电器模块接线:
  VCC         →   5V (或3.3V，取决于继电器型号)
  GND         →   ESP32-C3 GND
  IN          ←   GPIO12
  COM         →   高压发生器开关的一个对角接线柱
  NO (常开)   →   高压发生器开关的另一个对角接线柱
...
1. **电气安全**
   ...
   - **MOSFET保护**：MOSFET方案存在电流倒灌风险，可能导致ESP32死机
     - 推荐方案：使用光耦隔离MOSFET驱动信号
     - 简易方案：改用继电器模块（内置光耦隔离）
   - 高压发生器必须正确接地
...
| ESP32频繁重启 | 电源不足 | 使用独立5V电源供电 || **ESP32触发时死机/重启** | **MOSFET电流倒灌** | **改用继电器模块或添加光耦隔离** |
## 📁 项目结构
